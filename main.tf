module "vpc" {
  ...
  vpc_settings = {
    ...
    include_eks_tags = { # Optional, Required for EKS cluster.
      ...
      shared_or_owned = var.vpc.include_eks_tags.shared_or_owned # Defaults to "owned" is used for a single k8s ingress service (single alb endpoint per ingress), that will then use nginx controller as shared
    }
